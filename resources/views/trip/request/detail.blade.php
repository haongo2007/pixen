...
@section('content')
    ...
    <div class="container-fluid head">
        <div class="avatar">
            <img src="{{ asset($request->Avatar_user()) }}" alt="">
        </div>
        <h4 class="text-capitalize">{{ $request->Name_user() }}</h4>
        <div class="rate">
        ...
        <div class="row no-gutters detail__wrap">
            <div class="col-12 body">
                <div class="emp_luggage">
                    <b>Request : </b>
                    <b>{{ ucwords($request->Package->size) }} (kg) </b>
                </div>
                <div class="sec row">
                    <div class="col">
                ...
                <p>{{ $request->Package->description }}</p>
            </div>
            @if(Auth::user()->id == $request->Trip->user_id)
                <div class="d-flex w-100 justify-content-space-evenly">
                    <a href="{{ route('request.decline',['id'=>$request->id,'redirect' => 'trip','to'=> $request->trip_id]) }}" class="d-contents"><button class="btn btn-dark w-40">Decline</button></a>
            ...
            <div class="col-12 mt-4 request">
                ...
                <div class="cpn-ins d-flex">
                    ...
                    <img class="send_cmt ml-3 cursor" src="{{ asset('images/icon/next.svg') }}" width="20" uid="{{ Auth::user()->id }}" rid="{{ $request->id }}">
                </div>
            </div>
        </div>
    </div>
    <div class="modal fade" tabindex="-1" role="dialog" id="modal-create-invoice">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('trip.create.invoice',['id'=>$request->id]) }}" method="post" accept-charset="utf-8">
                    @csrf
